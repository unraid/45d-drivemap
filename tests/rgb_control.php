...
check($comet[HOMELAB_PINWHEEL_ORDER[0]] === HOMELAB_BRAND_ORANGE &&
  ...
  $comet[HOMELAB_PINWHEEL_ORDER[7]] !== '000000' &&
  $comet[HOMELAB_PINWHEEL_ORDER[8]] === '000000',
  'comet has orange head and eight-LED fading blue tail');
$comet_tail = array_map(fn($index) => $comet[$index], array_slice(HOMELAB_PINWHEEL_ORDER, 1, 7));
check(count(array_filter($comet_tail, fn($color) => $color !== '000000')) === 7 &&
  hexdec(substr($comet_tail[0], 0, 2)) > hexdec(substr($comet_tail[1], 0, 2)) &&
  hexdec(substr($comet_tail[1], 4, 2)) === 0 &&
  hexdec(substr($comet_tail[2], 0, 2)) > 0 && hexdec(substr($comet_tail[2], 4, 2)) > 0 &&
  hexdec(substr($comet_tail[3], 0, 2)) > 0 && hexdec(substr($comet_tail[3], 4, 2)) > 0 &&
  hexdec(substr($comet_tail[4], 0, 2)) === 0 && hexdec(substr($comet_tail[6], 4, 2)) > 0,
  'comet changes from orange to blue halfway through its eight-LED tail');
$short_comet = homelab_stream_leds(['effect' => 'comet-loop', 'tail_leds' => 3], 0);
check(hexdec(substr($short_comet[HOMELAB_PINWHEEL_ORDER[1]], 0, 2)) > 0 &&
  hexdec(substr($short_comet[HOMELAB_PINWHEEL_ORDER[1]], 4, 2)) > 0,
  'comet color transition stays centered when the tail length changes');
...
check(count(array_filter($eyes_closing, fn($color) => $color !== '000000')) === 8 &&
  count(array_filter($eyes_slit, fn($color) => $color !== '000000')) === 4 &&
  $eyes_closing[3] === '000000' && $eyes_closing[9] === '000000' &&
  $eyes_closing[0] !== '000000' && $eyes_closing[6] !== '000000' &&
  $eyes_fading[3] !== HOMELAB_BRAND_ORANGE && $eyes_fading[3] !== '000000',
...
check(homelab_eye_closure(2.0, 6) !== homelab_eye_closure(8.0, 6) &&
  ...
  'blink timing varies between cycles and uses the selected interval and eye color');
...
$eyes_positive = homelab_stream_leds(['effect' => 'halloween-eyes', 'eye_angle_degrees' => 45], 2.2);
$eyes_negative = homelab_stream_leds(['effect' => 'halloween-eyes', 'eye_angle_degrees' => -45], 2.2);
check(array_slice($eyes_positive, 0, 12) !== array_slice($eyes_positive, 12, 12) &&
  array_slice($eyes_positive, 0, 12) === array_slice($eyes_negative, 12, 12) &&
  array_slice($eyes_positive, 12, 12) === array_slice($eyes_negative, 0, 12) &&
  homelab_stream_leds(['effect' => 'halloween-eyes', 'eye_angle_degrees' => 0], 2.2) === $eyes_closing,
  'eye angle tilts the two slits in opposite directions and zero keeps them parallel');
...
foreach ([['period_seconds' => '0'], ['eye_angle_degrees' => '-61'],
  ['eye_angle_degrees' => '61'], ['eye_angle_degrees' => 'half'],
  ['brightness_pct' => ['100']],
...
check(strpos($page, 'Lighting mode') !== false && strpos($page, 'Separate fan colors') !== false &&
  ...
  strpos($page, 'name="eye_angle_degrees" min="-60" max="60" step="5"') !== false &&
